...
NodePtr Director::buidObject(INodeFactory & f)
{
    ...
    NodePtr adap(new MapAdapterNode(mapObject));

	std::shared_ptr<Node> obj = f.createGroup();
	obj->add(f.createPair("params1", adap) );
    obj->add(f.createPair("params2", adap->clone()) );
...
{
    std::shared_ptr<Node> obj = f.createGroup();
    obj->add( f.createPair("title", f.createString("Header")) );
...
NodePtr BodyBuilder::build(INodeFactory & f)
{
    ...
    return f.createPair("body", f.createObj(obj));
}
